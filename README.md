...
# Overview
This **vieW512** is a gadget for image viewing designed for Windows Sidebar. 
...
(If it is Windows Vista or later, I think that it can be executed on both 7 and 8.)
...
# はじめに
このソフトはStable Diffusionなどで大量に作成した画像の閲覧や、イラストを描く時の参考画像を表示させておくためのビューワー、また普通のフォトフレーム用として作成しました。
ご活用いただけると幸いです。

# 概要
このソフトはウィンドウズのサイドバー用の画像閲覧用ガジェットです。
※ご利用になるには、旧来のサイドバーガジェットが動くようにするため
<8GadgetPack>https://8gadgetpack.net/
などのサイドバーを復活させるソフトを予めインストールしておく必要があります。
また、ガジェットは仕様が古いためwebpなどの画像形式には対応していません。
（表示可能な画像形式は、BMP,GIF,JPEG,PNG形式です。）

# インストール方法
通常のガジェットと同じで、サイドバーが動いている環境であれば、ダブルクリックでインストールされます。

# アンインストール方法
右クリックからガジェットメニューウィンドウを出して、アイコンの右クリックメニューからアンインストールを選択します。

# 使用方法
マウスを画面中央に持っていくと、スライドショーの再生／停止と前後の画像に切り替えるボタンが表示されます。
画面上部では、表示倍率を設定できます。画面のダブルクリックで当該フォルダを開けます。
細かな設定はガジェット右上に表示される工具マーク「オプション」から各種設定が可能です。表示させたい画像のフォルダ指定などはこのオプション画面から指定してください。
また、一部機能については、キーボードでの操作もできます。
矢印キーの左右で前後画像切り替え。矢印キーの下で再生／停止。矢印キーの上で当該画像を標準画像アプリで表示。
...
# 免責事項
なお、製作者は、当ソフトウェアの利用によって生じたデータの損失や他の損害に対して一切の責任を負いません。
これには、直接的な損害、間接的な損害、偶発的な損害、結果的な損害も含まれます。
ご利用前に、この免責事項を十分に理解し、自己の責任でソフトウェアを使用してください。
